...
Accepts base64-encoded person + garment images, returns base64 result.
Weights are downloaded once at cold start and cached in /app/weights.
"""
import runpod
import logging
import sys
...
def download_weights():
    ...
    if not os.path.exists(tryon_path):
        ...
        log.info("TryOnModel weights already cached")
    ...
    for fname in ["yolox_l.onnx", "dw-ll_ucoco_384.onnx"]:
        ...
        if not os.path.exists(fpath):
            ...
            log.info(f"DWPose/{fname} already cached")

    # FashnHumanParser (auto-downloads to HF cache)
    log.info("Ensuring FashnHumanParser weights are cached...")
...
def handler(job):
    ...
    log.info(f"Inference completed in {inference_time:.2f}s")
